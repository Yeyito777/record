...
describe("timeline rendering", () => {
  ...
  test("renders reply preview role mentions as muted colored role names", () => {
    const timeline = createTimelineState();
    setTimelineRenderContext(
      timeline,
      "viewer",
      false,
      { "guild-1": [{ id: "role-1", name: "Block Tales", color: 0x00aaff, position: 1 }] },
      {},
      0,
      "guild-1",
    );
    setTimelineMessages(timeline, "channel-1", [message("message-1", "reply body", {
      ...
      reply: {
        ...
        authorDisplayName: "Alice",
        timestamp: Date.UTC(2026, 0, 1, 11, 59, 0),
        summary: "<@&role-1> who wants to play block tails",
    ...
    expect(stripAnsi(rendered.lines[0] ?? "")).toContain("@Block Tales who wants to play block tails");
    expect(stripAnsi(rendered.lines[0] ?? "")).not.toContain("<@&role-1>");
    expect(rendered.lines[0]).toContain(`${ansiTrueColor(0x00aaff)}@Block Tales${theme.muted}`);
  });
  ...
  test("replaces pending image uploads by nonce when Discord changes attachment metadata", () => {
    const timeline = createTimelineState();
    const pending = message("local-1", "caption", { authorId: "viewer", authorName: "Paramount" });
    pending.localStatus = "pending";
    pending.nonce = "123456789";
    ...
    echoed.nonce = "123456789";
    echoed.attachments = [{ id: "attachment-1", filename: "image.png", contentType: "image/png", size: 1492, url: "https://cdn.example/image.png" }];
    appendTimelineMessage(timeline, echoed);

    expect(timeline.messages).toHaveLength(1);
    expect(timeline.messages[0]).toMatchObject({ id: "real-1", content: "caption" });
    expect(timeline.messages[0]?.attachments[0]?.url).toBe("https://cdn.example/image.png");
    expect(timeline.messages[0]?.localStatus).toBeUndefined();
  });
